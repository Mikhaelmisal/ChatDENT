...
import 'package:chatdent/services/login.dart' show login, onLogoutCallbacks;
...
class DicomController {
  ...
  /// calendar day as an existing appointment for [chatdentPatientId].
  Set<DateTime> matchingDates(DicomPendingImport pi, String chatdentPatientId) {
    final apptDays = _appointmentDaySet(chatdentPatientId);
    return pi.dates
  ...
  Set<DateTime> mismatchingDates(DicomPendingImport pi, String chatdentPatientId) {
    final apptDays = _appointmentDaySet(chatdentPatientId);
    return pi.dates
        .where((d) => !apptDays.contains(DateTime(d.year, d.month, d.day)))
        .toSet();
  ...
  Set<DateTime> _appointmentDaySet(String chatdentPatientId) {
    final all =
        appointments.byPatient[chatdentPatientId]?["all"] ?? const <Appointment>[];
    return all
  ...
  /// already has a confirmed link to an ChatDENT patient).
  ///
  ...
  /// [chatdentPatientId]-overridden) patient's appointments. Removes the batch
  ...
  Future<void> approve(DicomPendingImport p, {String? chatdentPatientId}) async {
    if (isImporting) return;
    try {
      final result = await _importer.approveImport(
        p,
        chatdentPatientId: chatdentPatientId,
      );
      if (result.complete) {
  ...
  /// Manually sets the matched ChatDENT patient for [p] (the dentist picked one
  /// from the patient picker). Marks the match as confirmed and persists it
  /// so it survives app restarts (without auto-importing).
  Future<void> manualMatch(DicomPendingImport p, String chatdentPatientId) async {
    final matched = patients.get(chatdentPatientId);
    if (matched == null) return;
    p.matchedPatient = matched;
    p.confidence = 1.0;
    p.isConfirmed = true;
    await dicomPendingMatches.set(p.dicomPatientId, chatdentPatientId);
  ...
  /// The current DICOM→ChatDENT link map, for the "Linked Patients" section.
