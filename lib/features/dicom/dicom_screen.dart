...
import 'package:chatdent/features/dicom/dicom_auto_import_toggle.dart';
import 'package:chatdent/features/dicom/dicom_controller.dart';
import 'package:chatdent/features/dicom/dicom_dir_manager.dart';
import 'package:chatdent/features/dicom/open_dicom_viewer_panel.dart';
...
class _PendingImportCardState extends State<_PendingImportCard> {
  ...
  String? _buildDateMatchInfo(String chatdentId) {
    final matching = dicomCtrl.matchingDates(widget.pending, chatdentId);
    final mismatching = dicomCtrl.mismatchingDates(widget.pending, chatdentId);
...
class _DateAlignmentDialog extends StatelessWidget {
  const _DateAlignmentDialog({
    required this.pending,
    required this.chatdentId,
    required this.onConfirm,
    required this.onBack,
  });
  final DicomPendingImport pending;
  final String chatdentId;
  final VoidCallback onConfirm;
  final VoidCallback onBack;

  @override
  Widget build(BuildContext context) {
    final matching = dicomCtrl.matchingDates(pending, chatdentId);
    final mismatching = dicomCtrl.mismatchingDates(pending, chatdentId);
    final patient = patients.get(chatdentId);
...
class _LinkedPatientRow extends StatelessWidget {
  const _LinkedPatientRow({required this.dicomId, required this.chatdentId});
  ...
  Widget build(BuildContext context) {
    final theme = FluentTheme.of(context);
    final chatdentPatient = patients.get(chatdentId);
    final chatdentName = chatdentPatient?.title ?? txt('dicomPatientDeleted');
    final linked = chatdentPatient != null;
