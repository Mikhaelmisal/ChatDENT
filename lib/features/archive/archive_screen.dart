...
import 'package:chatdent/features/notes/dialog_column_edit.dart';
import 'package:chatdent/features/notes/dialog_note_edit.dart';
...
class _ArchivedPageState extends State<_ArchivedPage> {
  ...
  List<List<String>> get _sortKeys {
    ...
    if (_canAccess(Perm.leads)) keys.addAll(_collect("leads", leads));
    ...
    switch (key[1]) {
      ...
      case "leads":
        ...
        return _ArchivedRow(
          ...
          subtitle: l.sourceLabel,
          onRestore: () => leads.unarchive(l.id),
          onOpen: () => openLead(l),
        );
      case "notes":
  ...
  Widget _buildStoreSelector() {
    ...
    if (_canAccess(Perm.leads)) {
      items.add(ComboBoxItem(
        value: "leads",
        child: Row(mainAxisSize: MainAxisSize.min, spacing: 4, children: [
          Icon(FluentIcons.people, size: 14, color: Colors.teal),
          Txt(txt("leads")),
        ]),
      ));
    }
