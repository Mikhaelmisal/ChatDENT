...
/// PocketBase `data.id` is a 15-character `[a-zA-Z0-9_]` text field.
/// One longer id in a batch makes PocketBase reject the whole transaction.
@visibleForTesting
bool isPocketBaseRecordId(String id) {
  return id.length == 15 && RegExp(r'^[a-zA-Z0-9_]+$').hasMatch(id);
}

/// The `data` column is JSON. Local stores keep it as an encoded string.
@visibleForTesting
dynamic decodeStoredJsonField(String raw) {
...
class SaveRemote {
  ...
  Future<bool> put(List<RowToWriteRemotely> data) async {
    ...
    final valid = <RowToWriteRemotely>[];
    for (final item in data) {
      final id = item.id.trim();
      if (!isPocketBaseRecordId(id)) {
        logger(
          'Skipping PocketBase write for $storeName id "$id" '
          '(length ${id.length}; must be exactly 15 [a-zA-Z0-9_])',
          null,
          2,
        );
        continue;
      }
      valid.add(RowToWriteRemotely(id: id, data: item.data)..store = item.store);
    }
    if (valid.isEmpty) {
      return true;
    }
    ...
    for (var i = 0; i < valid.length; i += 100) {
      chunks.add(valid.sublist(i, min(i + 100, valid.length)));
    }
    ...
    for (var chunk in chunks) {
      try {
        ...
        for (var item in chunk) {
          batchOperation.collection(dataCollectionName).upsert(body: {
            "store": storeName,
            "data": decodeStoredJsonField(item.data),
            "id": item.id,
          });
        }
    ...
    if (predefinedMultipart != null) {
      ...
      multipart = http.MultipartFile.fromBytes(
        ...
        (await http.get(file!.path.startsWith("blob")
                ? Uri.parse(file.path)
                : Uri.parse(
                    'https://imgs.chatdent.app/?url=${Uri.encodeComponent(file.path)}')))
            .bodyBytes,
        filename: filename,
      );
    }
