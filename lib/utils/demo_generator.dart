...
import 'package:chatdent/features/expenses/expense_model.dart';
import 'package:chatdent/features/leads/lead_model.dart';
import 'package:chatdent/features/notes/notes_model.dart';
...
List<Lead> demoLeads(int length) {
  final rng = Random(7);
  return List.generate(length, (i) {
    final first = _firstNames[rng.nextInt(_firstNames.length)];
    final last = _lastNames[rng.nextInt(_lastNames.length)];
    final local =
        '${7 + rng.nextInt(3)}${List.generate(9, (_) => rng.nextInt(10)).join()}';
    return Lead.fromJson({
      "id": simpleHash("lead_$i", length: 15),
      "title": "$first $last",
      "phone": "91$local",
      "email": rng.nextBool() ? "${first.toLowerCase()}@example.com" : "",
      "source": LeadSource.all[rng.nextInt(LeadSource.all.length)],
      "campaign": rng.nextBool() ? "Spring ads ${2024 + rng.nextInt(3)}" : "",
      "interest": _leadInterests[rng.nextInt(_leadInterests.length)],
      "notes": rng.nextBool() ? "Called from ad form." : "",
      "stage": LeadStage.all[rng.nextInt(LeadStage.all.length)],
    });
  });
}
