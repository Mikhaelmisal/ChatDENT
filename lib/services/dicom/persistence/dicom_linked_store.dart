...
/// ```json
/// {"p": "chatdent-patient-uuid", "k": ["sop:...", "sop:..."]}
/// ```
///
/// - `p` is `null` until the dentist links the DICOM patient to an ChatDENT
...
class DicomLinksStore {
  ...
  Future<void> setPatient(String dicomPatientId, String chatdentPatientId) {
    return _serializeMutation(() async {
      try {
        final box = await _openBox;
        final current = _cache[dicomPatientId];
        final next = _LinkEntry(
          patient: chatdentPatientId,
          keys: [...?current?.keys],
        );
        await box.put(dicomPatientId, next.toJson());
        _cache[dicomPatientId] = next;
  ...
  ///
  /// A confirmed DICOM-to-ChatDENT patient mapping must survive independently
