...
import 'package:chatdent/utils/safe_hive_init.dart';
import 'package:hive_flutter/adapters.dart';

/// Hive-backed map of dentist-confirmed manual matches that haven't been
/// approved yet.  Schema: `dicomPatientId → chatdentPatientId` (both strings).
///
/// Used at scan time as a hint: if a DICOM patient has a pending match AND
/// the matched ChatDENT patient still exists AND actual `.dcm` files are found
/// on disk, the pending import surfaces with confidence 1.0.  If the files
/// were deleted or the ChatDENT patient was removed, the match is silently
...
class DicomPendingMatches {
  ...
  Future<void> set(String dicomPatientId, String chatdentPatientId) async {
    try {
      final box = await _openBox;
      await box.put(dicomPatientId, chatdentPatientId);
    } catch (e, s) {
