...
import 'package:chatdent/services/dicom/persistence/dicom_matched_store.dart';
import 'package:chatdent/services/dicom/persistence/dicom_unmatched_store.dart';
import 'package:chatdent/services/dicom/dicom_file_cache.dart';
import 'package:chatdent/services/dicom/dicom_helpers.dart' as dcm_helpers;
import 'package:chatdent/services/dicom/dicom_io_service.dart';
import 'package:chatdent/services/dicom/dicom_normalize.dart';
import 'package:chatdent/services/dicom/dicom_skipped.dart';
import 'package:chatdent/utils/imgs.dart' as imgs;
...
List<DicomPendingImport> _buildPending(
  ...
  for (final entry in groups.entries) {
    ...
    final linkedChatDENTId = dicomPatientId.isEmpty ? null : links[dicomPatientId];
    if (linkedChatDENTId != null && linkedChatDENTId.isNotEmpty) {
      final snap = patientSnapshots.firstWhere((p) => p.id == linkedChatDENTId,
          orElse: () => const _PatientSnapshot('', ''));
      if (snap.id == linkedChatDENTId) {
        matchedId = snap.id;
    ...
    if (matchedId == null && dicomPatientName.isNotEmpty) {
      ...
      if (pendingId != null && pendingId.isNotEmpty) {
        ...
        // If the ChatDENT patient was deleted, silently ignore the hint.
      }
    }
...
class DicomImporter {
  ...
  final Future<void> Function(String dicomPatientId, String chatdentPatientId)
      _setPatient;
  ...
  final List<Appointment> Function(String chatdentPatientId)
  ...
  final Future<DicomApprovalResult> Function(
          DicomPendingImport pending, String? chatdentPatientId)?
  ...
  DicomImporter({
    ...
    Future<void> Function(String dicomPatientId, String chatdentPatientId)?
        setPatient,
  ...
  Future<DicomApprovalResult> approveImport(
    ...
    String? chatdentPatientId,
  }) async {
    final override = approvalOverrideForTesting;
    if (override != null) {
      return override(pending, chatdentPatientId);
    }
    final targetId = chatdentPatientId ?? pending.matchedPatient?.id;
    ...
    log.info(
        'DicomImporter.approveImport: starting for dicomPatient="${pending.dicomPatientId}" chatdentPatient="$targetId" (${pending.files.length} files)');
