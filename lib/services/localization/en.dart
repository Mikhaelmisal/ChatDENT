enum Direction {
  ltr,
  rtl,
}

class En {
  Direction $direction = Direction.ltr;
  String $name = "English";
  String $code = "en";

  Map<String, String> dictionary = {
    // common
    "cancel": "Cancel",
    "close": "Close",
    "save": "Save",
    "reset": "Reset",
    "dicomViewer": "DICOM Viewer",
    "window": "Window",
    "colorMap": "Color Map",
    "invert": "Invert",
    "rotateLeft": "Rotate Left",
    "rotateRight": "Rotate Right",
    "loadingDicom": "Loading X-ray…",
    "dicomLoadFailed": "Failed to load X-ray",
    "defaultWindow": "Default",
    "controls": "Controls",
    "measure": "Measure",
    "metadata": "Metadata",
    "level": "Level",
    "width": "Width",
    "none": "None",
    "ruler": "Ruler",
    "roi": "ROI",
    "exportPng": "Export PNG",
    "measureInstructions": "Tap two points on the image to measure distance.",
    "roiInstructions":
        "Drag to draw a rectangular ROI. Statistics appear on release.",
    "clearMeasurement": "Clear measurement",
    "distance": "Distance",
    "mm": "mm",
    "roiStatistics": "ROI Statistics",
    "roiMean": "Mean",
    "roiStdDev": "StdDev",
    "roiMin": "Min",
    "roiMax": "Max",
    "roiMedian": "Median",
    "roiPixelCount": "Pixels",
    "fullRange": "Full Range",
    "dicomDefault": "DICOM Default",
    "mid50": "Mid 50%",
    "narrow25": "Narrow 25%",
    "high": "High",
    "low": "Low",
    "grayscale": "Grayscale",
    "hotIron": "Hot Iron",
    "petHeat": "PET Heat",
    "rainbow": "Rainbow",
    "cool": "Cool",
    "bone": "Bone",
    "continue": "Continue",
    "add": "Add",
    "edit": "Edit",
    "delete": "Delete",
    "deleteForever": "Delete forever",
    "deleteForeverHint":
        "This permanently removes the selected items. Enter the 4-digit passcode to continue.",
    "deletePasscode": "Delete passcode",
    "deletePasscode_desc":
        "4-digit code required to permanently delete archived items. Default is 1234.",
    "wrongDeletePasscode": "Wrong passcode",
    "deletedItems": "Deleted Items",
    "deleted": "Deleted",
    "archive": "Archive",
    "restore": "Restore",
    "logout": "Logout",
    "voiceInput": "Voice Input",
    "noResultsFound": "No results found",
    "startTyping": "Start typing",
    "home": "Home",
    "calendar": "Calendar",
    "gettingImages": "Getting images",
    "more": "More",
    "howToUse": "How to use",

    // new
    "newPatient": "New patient",
    "newLead": "New lead",
    "newAppointment": "New appointment",
    "newLabwork": "New labwork",
    "newNote": "New note",
    "newUser": "New user",
    "newAdmin": "New admin",

    // screens
    "settings": "Settings",
    "dashboard": "Dashboard",
    "collapseSidebar": "Show icons only",
    "expandSidebar": "Show names",
    "appointments": "Appointments",
    "patients": "Patients",
    "leads": "Leads",
    "doctors": "Doctors",
    "labworks": "Labworks",
    "insights": "Insights",

    // dashboard
    "admin": "Admin",
    "payments": "Payments",
    "appointmentsToday": "Appointments today",
    "patientsToday": "Patients today",
    "newPatientsToday": "New patients today",
    "paymentsMadeToday": "Payments made today",
    "newLeadsToday": "New leads today",
    "needsCall": "Need a call",
    "byNewest": "Newest",
    "addFirstLead": "Add a name and phone to capture a lead",
    "modeAdmin": "admin mode",
    "modeUser": "user mode",
    "modeOffline": "offline mode",
    "onceConnectionIsStable":
        "Once your connection becomes stable you can use the button in the top right corner to reconnect.",
    "unRestrictedAccess": "unrestricted access in the application",
    "restrictedAccess":
        "restricted access to some screens as dictated by the administrator",
    "youAreCurrentlyIn": "You are currently in",
    "mode": "mode",
    "youHave": "You have",
    "hello": "Hello",

    // datatable
    "archiveSelected": "Archive Selected",
    "searchPlaceholder": "Search...",
    "showHideArchived": "Show/Hide Archived",
    "showing": "Showing",
    "showMore": "Show more",
    "by": "By",
    "byName": "By name",
    "noItemsFound": "No items found",
    "filter": "Filter",

    // doctors
    "doctor": "Doctor",
    "pastAppointments": "Past appointments",
    "upcomingAppointments": "Upcoming appointments",
    "viewAllAppointments": "View all appointments",
    "otherAppointments": "Other appointments",
    "addAppointment": "Add Appointment",

    // days and months
    "monday": "Monday",
    "tuesday": "Tuesday",
    "wednesday": "Wednesday",
    "thursday": "Thursday",
    "friday": "Friday",
    "saturday": "Saturday",
    "sunday": "Sunday",

    // appointment card
    "appointment": "Appointment",
    "patient": "Patient",
    "photos": "Photos",
    "pre-opNotes": "Pre-op notes",
    "post-opNotes": "Post-op notes",
    "prescription": "Prescription",
    "pay": "Pay",
    "paid": "Paid",
    "price": "Price",
    "underpaid": "Underpaid",
    "overpaid": "Overpaid",
    "fullyPaid": "Fully paid",
    "includesOtherAppointments":
        "This total also reflects payments from other appointments.",
    "paymentSummary": "Payment summary",
    "after": "After",
    "days": "days",
    "day": "day",
    "isDone": "Appointment is done",
    "addLabwork": "Add labwork for this appointment",
    "labworksForThisAppointment": "Labworks for this appointment",

    // patients
    "patientDetails": "Patient Details",
    "age": "Age",
    "gender": "Gender",
    "lastVisit": "Last visit",
    "nextVisit": "Next visit",
    "totalPayments": "Total payments",
    "daysAgo": "days ago",
    "noVisits": "No visits",
    "name": "Name",
    "birthYear": "Birthdate",
    "birthdate": "Birthdate",
    "phone": "Phone",
    "noValidNumbersFound": "No valid phone numbers found",
    "phoneMustBe10Digits": "Phone number must be 10 digits",
    "theFollowingPhoneNumbersAreDetected":
        "Only the following phone numbers are detected",
    "email": "Email",
    "address": "Address",
    "notes": "Notes",
    "patientTags": "Patient tags",
    "male": "Male",
    "female": "Female",
    "noAppointmentsFound":
        "No appointments found for this patient, use the button below to add new appointment.",
    "patientPage": "Patient Web Page",
    "patientCanUseTheFollowing":
        "Patient can use the following link to access his appointments, payments and photos.",
    "web": "Web",
    "telegram": "Telegram",
    "printQR": "Print QR Code",
    "dentalNotes": "Dental Notes",
    "showPrimary": "Show Primary",
    "showPermanent": "Show Permanent",
    "primary": "Primary",
    "permanent": "Permanent",
    "lower": "Lower",
    "upper": "Upper",
    "left": "Left",
    "right": "Right",
    "centralIncisor": "Central Incisor",
    "lateralIncisor": "Lateral Incisor",
    "canine": "Canine",
    "firstPremolar": "First Premolar",
    "secondPremolar": "Second Premolar",
    "firstMolar": "First Molar",
    "secondMolar": "Second Molar",
    "thirdMolar": "Third Molar",
    "exportSelected": "Export Selected",
    "export": "Export",
    "exportImportFeatureExplanation":
        "To move selected patient and appointment data between servers or other apps, export a CSV from the source server and import it into the destination server or app.",
    "qrCode": "QR Code",
    "share": "Share",

    // communication
    "contact": "Contact",
    "sendEmail": "Send Email",
    "whatsapp": "WhatsApp",
    "call": "Call",
    "text": "Text",
    "countryCode": "Country code",
    "countryCode_desc":
        "Select the default country code when making calls or sending messages",

    // treatments
    "treatment": "Treatment",
    "allTreatments": "All Treatments",
    "extraction": "Extraction",
    "filling": "Filling",
    "pulpotomy": "Pulpotomy",
    "rCT": "RCT",
    "other": "Other",
    "re-RCT": "Re-RCT",
    "ortho": "Orthodontic",
    "whitening": "Whitening",
    "clean": "Cleaning",
    "implant": "Implant",
    "surgery": "Surgery",
    "crown": "Crown",
    "veneer": "Veneer",
    "bridge": "Bridge",
    "overlay": "Overlay",
    "temporary": "Temporary",
    "tap the treatment you have performed to register it to the tooth":
        "Tap the treatment you have performed to register it to the tooth",
    "tap the condition or history of the tooth to register it":
        "Tap the condition or history of the tooth to register it",
    "missing": "Missing",
    "caries": "Caries",
    "fractured": "Fractured",
    "mobility": "Mobility",
    "recession": "Recession",
    "rroot": "R. Root",
    "rprimary": "R. Primary",
    "malposition": "Malposition",
    "impacted": "Impacted",
    "abutment": "Abutment",
    "pontic": "Pontic",

    // calendar (appointments)
    "allDoctors": "All doctors",
    "noAppointmentsForThisDay": "No appointments for this day",
    "weekAbbr": "W",
    "twoWeeksAbbr": "2W",
    "monthAbbr": "M",
    "min": "min",
    "minutes": "minutes",
    "duration": "Duration",
    "today": "Today",
    "switchToTimelineView": "Switch to timeline view",
    "switchToAgendaView": "Switch to agenda view",
    "date": "Date",
    "changeDate": "Change Date",
    "changeTime": "Change Time",
    "time": "Time",
    "preOperativeNotes": "Pre-operative notes",
    "selectDoctors": "Select Doctors",
    "selectPatient": "Select Patient",
    "operativeDetails": "Operative Details",
    "postOperativeNotes": "Post-operative notes",
    "extraInfoPlaceholder": "Extra info",
    "selectToothDetail": "Select what was found / what to do",
    "procedureTracking": "Procedure steps",
    "inProgress": "In progress",
    "completed": "Completed",
    "markTeethToTrackSteps":
        "Mark a treatment on the tooth chart to track steps here.",
    "workingLength": "Working length",
    "gpNumber": "GP number",
    "gpDiameter": "Diameter",
    "procTitleRCT": "Root canal",
    "stepAccessOpening": "Access opening",
    "stepBmp": "Biomechanical preparation",
    "stepObturation": "Obturation",
    "stepPor": "Post obturation restoration (POR)",
    "stepCrownCutting": "Crown cutting",
    "stepMeasurementsScan": "Measurements / scan",
    "stepCrownCementation": "Crown cementation",
    "stepAccessRestRemoval": "Access / restoration removal",
    "stepGpRemoval": "Gutta percha removal",
    "stepAnaesthesia": "Anaesthesia",
    "stepElevationExtraction": "Elevation and extraction",
    "stepHaemostasis": "Haemostasis",
    "stepPostExoInstructions": "Post-extraction instructions",
    "stepIsolationCaries": "Isolation and caries removal",
    "stepCavityPrep": "Cavity preparation",
    "stepRestorationPlaced": "Restoration placed",
    "stepFinishOcclusion": "Finishing and occlusion check",
    "stepPulpMedicament": "Pulp amputation and medicament",
    "stepOrthoRecords": "Records / scan / photos",
    "stepOrthoBonding": "Aligners issued / attachments / bonding",
    "stepOrthoActivation": "Review / next trays / activation",
    "stepInstructionsGiven": "Instructions given",
    "stepShadeRecorded": "Shade recorded",
    "stepWhiteningApplied": "Isolation and application",
    "stepWhiteningDone": "Whitening completed",
    "stepScaling": "Scaling",
    "stepPolishing": "Polishing",
    "stepOhi": "Oral hygiene instructions",
    "stepImplantPlanning": "Planning reviewed",
    "stepImplantPlaced": "Implant placed",
    "stepHealingAbutment": "Cover screw / healing abutment",
    "stepImplantRestored": "Implant restored",
    "stepAnaesthesiaFlap": "Anaesthesia and flap",
    "stepSurgicalProcedure": "Surgical procedure",
    "stepSuturing": "Suturing",
    "stepPostOpInstructions": "Post-operative instructions",
    "stepToothPrep": "Tooth preparation",
    "stepImpressionScan": "Impression / scan",
    "stepTemporaryCrown": "Temporary crown",
    "stepVeneerPrep": "Veneer preparation",
    "stepTemporariesPlaced": "Temporaries placed",
    "stepVeneerBonded": "Veneer bonded",
    "stepOverlayCemented": "Overlay cemented",
    "stepTemporaryPlaced": "Temporary restoration placed",
    "stepOcclusionChecked": "Occlusion checked",
    "stepAbutmentPrep": "Abutment preparation",
    "stepTemporaryBridge": "Temporary bridge",
    "stepBridgeCemented": "Bridge cemented",
    "stepPonticSpace": "Pontic space prepared",
    "stepPonticFitted": "Pontic fitted with bridge",
    "stepProcedureCompleted": "Procedure completed",
    "extraNotesFromAppointments": "Additional notes from appointments:",
    "otherAppointmentsNotes": "Other appointments notes:",
    "printPrescription": "Print Prescription",
    "priceIn": "Price in",
    "paidIn": "Paid in",
    "gallery": "Gallery",
    "link": "Link",
    "upload": "Upload",
    "camera": "Camera",
    "emptyGallery": "Empty gallery",
    "noPhotos": "No photos has been uploaded yet.",
    "otherPhotos": "Photos from other appointments",
    "dcm": "DCM",
    "generatingPreview": "Generating preview…",
    "draw": "Draw",
    "erase": "Erase",
    "showDrawings": "Show Drawings",
    "importingPhotosFromLink": "Importing photos from link",
    "useThisForm":
        "Use this form to import photos from share links, like google photos, or any accessible web page that contains photos you'd like to add to this appointment.",
    "error": "Error",
    "enterLink": "Enter a URL that contains photos",
    "import": "Import",

    // dicom
    "xrayLink": "X-Ray Link",
    "dicomSettings": "DICOM / X-Ray Settings",
    "dicomWatchDir": "X-Ray watch folder",
    "dicomWatchDir_desc":
        "Folder where your X-Ray sensor software saves .dcm files. ChatDENT scans it for new X-rays.",
    "dicomWatchDir_notSet": "No folder set — pick one to start importing",
    "dicomAutoImport": "Auto-import",
    "dicomAutoImport_desc":
        "Automatically import new X-rays for already-linked patients during scans.",
    "dicomResetViewerPrefs": "Reset viewer preferences",
    "dicomViewerPrefsDefault": "Using DICOM defaults",
    "dicomImportedFiles": "Imported files",
    "dicomLinkedPatients": "Linked patients",
    "scanNow": "Scan",
    "changeFolder": "Change folder",
    "dicomManageFolders": "Manage folders",
    "dicomWatchDirs_dialogTitle": "X-Ray Watch Folders",
    "dicomAddFolder": "Add folder",
    "dicomFolder": "Folder",
    "dicomFolders": "Folders",
    "dicomNoNewFiles": "No new X-rays found",
    "dicomNoNewFiles_desc":
        "All scanned files are already imported. New files will appear here on the next scan.",
    "dicomImportingFiles": "Importing {current} of {total} files…",
    "dicomUnknownPatient": "Unknown patient",
    "dicomPatientId": "DICOM ID",
    "dicomFiles": "files",
    "dicomDates": "dates",
    "dicomStudyDate": "Study date",
    "approve": "Approve",
    "changeMatch": "Change match",
    "unmatch": "Unmatch",
    "dicomShowFiles": "Show files",
    "dicomHideFiles": "Hide files",
    "confidence": "Confidence",
    "dicomAutoLinked": "auto-linked",
    "dicomNoMatch": "No matching patient found — pick one manually",
    "dicomSelectPatient": "Select patient matching",
    "dicomSearchPatient": "Search patient by name…",
    "dicomSearchLinkedPatients": "Search linked patients…",
    "dicomShowDeletedLinks": "Show deleted",
    "dicomNoMatchingLinks": "No linked patients match your search.",
    "dicomLinkActive": "Active",
    "dicomLinkBroken": "Broken",
    "dicomNoLinks": "No linked patients yet.",
    "dicomPatientDeleted": "Patient deleted",
    "dicomPatientDeleted_desc":
        "This ChatDENT patient no longer exists — unlink to re-match.",
    "dicomUnlink": "Unlink",
    "dicomScanSnapshotting": "Reading import registry…",
    "dicomScanListingDir": "Listing DICOM files…",
    "dicomScanScanningFiles": "Scanning files",
    "dicomScanPersisting": "Saving scan results…",
    "dicomScanMatching": "Matching patients…",
    "dicomScanParsing": "Parsing",
    "dicomScanCacheHit": "Cache hit",
    "dicomScanHalt": "Halt",
    "dicomSearchPending": "Search by patient name or DICOM ID…",
    "dicomFilterMatched": "Matched only",
    "dicomDatesMatch": "Dates match",
    "dicomDatesNoMatch": "No date match",
    "dicomDatesPartial": "Partial date match",
    "dicomDateAlignment": "Date alignment",
    "dicomMatchingPatient": "Matching patient",
    "dicomDatesMatchList": "These dates match existing appointments:",
    "dicomDatesNoMatchList": "These dates have no matching appointment:",
    "foundAppointment": "Found appointment",
    "willBeCreated": "Will be created",

    // labworks
    "laboratory": "Laboratory",
    "month": "month",
    "labwork": "labwork",
    "orderNotes": "Order notes",
    "noSuggestions": "No suggestions",
    "received": "Labwork received",
    "deliveredToPatient": "Delivered to patient",
    "showDone": "Show done",
    "waitingForLab": "waiting for lab",
    "undelivered": "Undelivered to patient",
    "receivedAndDelivered": "Received and delivered",
    "selectPatientFirst": "Select a patient first",
    "patientHasNoAppointments": "Patient has no appointments",
    "selectAppointment": "Select an appointment",
    "areYouSureYouWantTo": "Are you sure you want to",
    "youCanRestoreFromDeletedItemsPage":
        "You can restore from the deleted items page",
    "youWillNotBeAbleToRestore": "You will not be able to restore",

    // expenses
    "expense": "Expense",
    "expenses": "Expenses",
    "receipt": "Receipt",
    "title": "Title",
    "cost": "Cost",
    "amount": "Amount",
    "status": "Status",
    "due": "Due",
    "open": "Open",
    "rename": "Rename",
    "addSupplier": "Add Supplier",
    "items": "Items",
    "markAsPaid": "Mark as paid",
    "markAsDue": "Mark as due",
    "addOrder": "Add order",
    "new": "New",
    "thisMonth": "This month",
    "addPhoto": "Add photo",
    "files": "Files",
    "noFiles": "No files have been added yet.",
    "change": "Change",
    "empty": "Empty",
    "totalDue": "Total due",
    "notSet": "Not set",
    "photo": "Photo",
    "viewAllOrders": "View all orders",
    "lastOrder": "Last order",
    "supplier": "Supplier",
    "order": "Order",
    "orders": "Orders",

    // notes
    "addColumn": "Add column",
    "editColumn": "Edit column",
    "columnTitle": "Column title",
    "columnColor": "Column color",
    "addNote": "Add note",
    "editNote": "Edit note",
    "note": "Note",
    "recurrence": "Recurrence",
    "isARecurrenceOfOlderNote": "Is a recurrence of an older note",
    "recurringEvery": "Recurring every",
    "recurring": "Recurring",
    "column": "Column",
    "uncategorized": "Uncategorized",
    "relatingToPatient": "Relating to patient",
    "assignedTo": "Assigned to",
    "dueDate": "Due date",
    "attachments": "Attachments",
    "deleteAllCompletedNotes": "Delete all completed notes",
    "moveTowardsEnd": "Move towards end",
    "moveTowardsStart": "Move towards start",
    "item": "Item",
    "addAttachment": "Add attachment",
    "tapToDownload": "Tap to download",
    "addComment": "Add comment",
    "sendComment": "Send comment",
    "tapToMarkAsPending": "Tap to mark as pending",
    "tapToMarkAsDone": "Tap to mark as done",
    "archived": "Archived",
    "noDeletedItems": "No deleted items",
    "overdue": "Overdue",
    "incoming": "Incoming",
    "outgoing": "Outgoing",
    "pending": "Pending",
    "completed": "Completed",
    "unassigned": "Unassigned",
    "comments": "Comments",
    "createdDate": "Created date",
    "changeDueDate": "Change due date",
    "createdBy": "Created by",
    "switchAssignee": "Switch assignee",
    "noAssignee": "No assignee",
    "sort": "Sort",
    "ascending": "ascending",
    "descending": "descending",
    "showAll": "Show all",
    "allAccounts": "All accounts",

    // stats
    "pickRange": "Pick range",
    "pickPhotos": "Pick photos",
    "pickFiles": "Pick files",
    "photoSlideshow": "Photo slideshow",
    "start": "Start",
    "end": "End",
    "weeks": "Weeks",
    "months": "Months",
    "quarters": "Quarters",
    "years": "Years",
    "year": "Year",
    "appointmentsPer": "Appointments per",
    "paymentsAndExpensesPer": "Payments and expenses per",
    "newPatientsPer": "New patients per",
    "doneMissedPer": "Done & Missed per",
    "timeOfDay": "Time of day",
    "dayOfWeek": "Day of week",
    "dayOfMonth": "Day of month",
    "monthOfYear": "Month of year",
    "patientsGender": "Patients gender",
    "total": "Total",
    "in_Duration_": "in",
    "acquiredPatientsIn": "Acquired patients in",
    "doneAndMissedAppointmentsIn": "Done and missed appointments in",
    "distributionOfAppointments": "Distribution of appointments",
    "maleAndFemalePatients": "Male and female patients",
    "done": "done",
    "missed": "missed",
    "periodicity": "Periodicity",

    // settings
    "currency": "Currency",
    "currency_desc":
        "Currency code to be used across the application (e.g. USD, EUR, IQD).",
    "phone_desc":
        "Primary number patients should call for appointments and enquiries. Also used as {phone} in WhatsApp templates, prescriptions, and the patient web page.",
    "prescriptionFooter": "Prescription footer",
    "prescriptionFooter_desc":
        "This text will be displayed at the bottom of the prescription.",
    "language": "Language",
    "language_desc":
        "The interface language for the menus, buttons, and info used across the app.",
    "startingDayOfWeek": "Starting day of week",
    "startingDayOfWeek_desc": "The day of the week that the week starts on.",
    "dateFormat": "Date format",
    "dateFormat_desc":
        "This format will be used to display dates where applicable.",
    "calendarView": "Calendar view",
    "calendarView_desc":
        "Choose how appointments are displayed in the calendar.",
    "agenda": "Agenda (list)",
    "timeline": "Timeline (time grid)",
    "cacheReset": "Cache reset",
    "cacheReset_desc":
        "Using the button below, you can delete locally saved data (except for images), this would solve some issues with synchronization if you're facing any.",
    "month/day/year": "Month/Day/Year",
    "day/month/year": "Day/Month/Year",
    "appliesTo": "Applies to",
    "all": "All",
    "you": "you",
    "system": "system",
    "initialSynchronization": "Initial synchronization",
    "synchronizing": "Synchronizing",
    "clearingLocalData": "Clearing local data",
    "dentalNotation": "Dental notation",
    "dentalNotation_desc": "choose your preferred system of notation",
    "palmer": "Palmer",
    "universal": "Universal",
    "iso": "ISO",

    // advanced settings: Backups
    "backup": "Backup",
    "backups": "Backups",
    "refresh": "Refresh",
    "download": "Download",
    "createNew": "Create new",
    "restoreBackup": "Restore backup",
    "restoreBackupWarning1":
        "Restoring this backup will overwrite all data in the app currently. Any changes that you have made after the date of this backup",
    "restoreBackupWarning2":
        "will be lost.\n\nAre you sure you want to restore this backup?",
    "sureDeleteBackup": "Are you sure you want to delete this backup",
    "backupDate": "Backup date",
    "useTheFollowingLinkToDownloadTheBackup":
        "Use the following link to download the backup",
    "backups_config_title": "Backup Configuration",
    "backups_config_desc":
        "Configure automatic backup schedule and storage settings.",
    "backups_auto_enabled": "Enable automatic backups",
    "backups_cron": "Schedule (cron)",
    "backups_cron_presets": "Presets",
    "backups_cron_hourly": "Every hour",
    "backups_cron_6hours": "Every 6 hours",
    "backups_cron_12hours": "Every 12 hours",
    "backups_cron_daily": "Daily at midnight",
    "backups_cron_daily3am": "Daily at 3 AM",
    "backups_cron_weekly": "Weekly (Sun midnight)",
    "backups_cron_monthly": "Monthly (1st midnight)",
    "backups_cron_hint":
        "Select a schedule preset from the dropdown. The cron expression is auto-filled.",
    "backups_max_keep": "Max backups to keep",
    "backups_max_keep_hint": "Older backups will be deleted when exceeded",
    "backups_s3_enabled": "Store backups in S3",
    "backups_save_success": "Backup settings saved successfully",
    "backups_save_fail": "Failed to save backup settings",

    // Meta settings
    "meta_settings": "Application Settings",
    "meta_info_title": "General Application Info",
    "meta_info_desc":
        "Configure your application name, URL, and branding settings.",
    "meta_appName": "Application Name",
    "meta_appName_hint": "Shown in emails and the login screen title",
    "meta_appUrl": "Application URL",
    "meta_appUrl_hint": "Used in emails and shared links",
    "meta_save_success": "Application settings saved successfully",
    "meta_save_fail": "Failed to save application settings",

    // Auth settings
    "auth_settings": "Authentication Settings",
    "auth_info_title": "Session Duration",
    "auth_info_desc":
        "Set how long authentication tokens remain valid before users need to log in again.",
    "auth_users_duration": "User session (days)",
    "auth_users_duration_hint":
        "How many days before regular users need to re-login",
    "auth_superusers_duration": "Admin session (days)",
    "auth_superusers_duration_hint":
        "How many days before admins need to re-login",
    "auth_save_success": "Authentication settings saved successfully",
    "auth_save_fail": "Failed to save authentication settings",

    // File upload settings
    "file_upload_settings": "File Upload Settings",
    "file_upload_info_title": "Maximum File Upload Size",
    "file_upload_info_desc":
        "Set the maximum allowed size for file uploads (images, documents, etc.) across the application.",
    "file_upload_max_size": "Maximum file size (MB)",
    "file_upload_max_size_hint":
        "Maximum size in megabytes for each uploaded file",
    "file_upload_save_success": "File upload settings saved successfully",
    "file_upload_save_fail": "Failed to save file upload settings",

    // advanced settings: Admins
    "admins": "Administrators",
    "validEmailMustBeProvided": "A valid email must be provided",
    "password": "Password",
    "leaveBlankToKeepUnchanged": "Leave blank to keep unchanged",
    "updatingPassword": "Updating password",
    "leaveItEmpty":
        "Leave the password field empty if you don't want to change it.",
    "minimumPasswordLength": "Minimum 10 characters password",

    // advanced settings: Users
    "users": "Users",

    // advanced settings: Permissions
    "permissions": "User Permissions",
    "restricted": "Restricted",
    "personal": "Personal",
    "full": "Full",
    "view": "View",
    "local": "local",
    "canUpload": "Can upload",
    "cantUpload": "Can not upload",
    "revenue": "Revenue",
    "canSee": "Can see",
    "canNotSee": "Can not see",
    "accounts": "ِAccounts",
    "account": "Account",
    "user": "User",
    "operatesOnPatients": "Operates on patients",

    // S3 settings
    "s3_settings": "S3 Storage Settings",
    "s3_info_title": "Configure S3 Storage",
    "s3_info_desc":
        "Configure your S3-compatible storage settings for file uploads and backups.",
    "s3_endpoint": "Endpoint URL",
    "s3_bucket": "Bucket Name",
    "s3_region": "Region",
    "s3_accessKey": "Access Key",
    "s3_secretKey": "Secret Key",
    "s3_secretKey_hint":
        "Leave empty to keep unchanged. Your S3 secret access key",
    "s3_enabled": "Enable S3 Storage",
    "s3_forcePathStyle": "Force path style",
    "s3_forcePathStyle_hint":
        "Use path-style addressing instead of subdomain (needed for Cloudflare R2)",
    "s3_endpoint_hint": "Your S3 provider's endpoint URL",
    "s3_bucket_hint": "The name of your S3 bucket",
    "s3_region_hint": "Bucket region, or 'auto' for Cloudflare R2",
    "s3_accessKey_hint": "Your S3 access key ID",
    "s3_test_success": "S3 configuration is working properly",
    "s3_test_fail": "S3 configuration test failed",
    "s3_save_success": "S3 settings saved successfully",
    "s3_save_fail": "Failed to save S3 settings",
    "test": "Test",

    // SMTP settings
    "smtp_settings": "SMTP Email Settings",
    "smtp_info_title": "Configure SMTP Email",
    "smtp_info_desc":
        "Configure your SMTP email settings for sending emails (password resets, notifications, etc.).",
    "smtp_host": "Host",
    "smtp_port": "Port",
    "smtp_username": "Username",
    "smtp_password": "Password",
    "smtp_senderName": "Sender Name",
    "smtp_senderEmail": "Sender Email",
    "smtp_tls": "Use TLS",
    "smtp_tls_auto": "Auto (STARTTLS)",
    "smtp_tls_always": "Always",
    "smtp_tls_hint": "Auto = STARTTLS (port 587)\n Always = SSL (port 465)",
    "smtp_presets": "Presets",
    "smtp_port_blocked_warning":
        "Some cloud hosts (e.g. DigitalOcean, Hetzner, Google Cloud) block outgoing SMTP ports by default. Make sure 587 or 465 is allowed, or use a different port.",
    "smtp_host_hint": "Your email provider's SMTP server address",
    "smtp_port_hint": "578 or 465",
    "smtp_username_hint": "Usually your full email address",
    "smtp_password_hint":
        "Leave empty to keep unchanged. Use an app-specific password if 2FA is enabled",
    "smtp_app_passwords": "App passwords",
    "smtp_senderName_hint": "Display name shown in the From field",
    "smtp_senderEmail_hint": "Email address shown in the From field",
    "smtp_localName": "HELO/EHLO Local Name",
    "smtp_localName_hint": "Used in HELO/EHLO greeting, usually your domain",
    "smtp_enabled": "Enable SMTP",
    "smtp_test_success": "SMTP configuration is working properly",
    "smtp_test_fail": "SMTP configuration test failed",
    "smtp_save_success": "SMTP settings saved successfully",
    "smtp_save_fail": "Failed to save SMTP settings",

    "fail": "Fail",
    "success": "Success",

    // login
    "login": "Login",
    "clear": "Clear",
    "resetPassword": "Reset password",
    "serverUrl": "Server URL",
    "proceedOffline": "Proceed offline",
    "youLLGet": "Get reset link by email",
    "beenSent":
        "Password reset instructions have been sent to you, check your inbox.",

    // static notifications flyout
    "notifications": "Notifications",
    "whatsappConnected": "WhatsApp connected",
    "whatsappConnecting": "WhatsApp connecting",
    "whatsappDisconnected": "WhatsApp not connected",
    "enableNotifications": "Enable notifications",
    "clearAllNotifications": "Clear all",
    "dueLabworks": "Due labworks",
    "undeliveredLabworks": "Undelivered labworks",
    "outgoingNotes": "Outgoing notes",
    "incomingNotes": "Incoming notes",
    "appointmentsSetToday": "appointments set today",
    "labworksDue": "labworks due",
    "labworksNotDelivered": "labworks not delivered",
    "outgoingPendingNotes": "outgoing pending notes",
    "incomingPendingNotes": "incoming pending notes",

    // patientSide
    "generateQRLink": "Generate QR Code",
    "patientSide": "Patient Side",

    // other
    "sureClosePanel":
        "Are you sure you want to close this panel? Any unsaved changes will be lost.",
    "sure": "Sure",
    "back": "Back",
    "firstLaunchDialogTitle": "Welcome to ChatDENT",
    "firstLaunchDialogContent":
        "ChatDENT is installed. Open Settings to set clinic hours, the primary phone, Evolution WhatsApp, and templates. The install steps are in README.md and manual.md in this project.",
    "newVersionDialogTitle": "New version available",
    "newVersionDialogContent":
        "A new version of ChatDENT is available. Please update to the latest version to get the best experience. Scan the QR code below to get started.",
    "changelogDialogTitle": "What's New in",

    // login required
    "loginRequired": "Login required",
    "loginRequiredDesc": "Please login to continue",

    // server help flyout
    "whatIsAServer": "What is a server?",
    "helpOnCreatingAServer":
        "A server is PocketBase in Docker on this clinic PC (http://localhost:8095). Follow README.md to start it.",
    "phoneNeedsLanServer":
        "On a phone, Server cannot be localhost. Use the clinic PC Wi‑Fi address, e.g. http://192.168.1.20:8095 (ipconfig on the PC).",
    "createNewServer": "Create new server",

    // errors
    "errors": "Errors",
    "reconnect": "Reconnect",
    "uploadingAttachment": "uploading attachment",
    "errorHappenedWhen": "Error happened when",
    "deletingPatientImageFromServer": "deleting patient image from server",
    "importingRemoteImages": "importing remote images",
    "creatingNewAccount": "creating new account",
    "updatingAccounts": "updating accounts",
    "deletingAccount": "deleting account",
    "uploadingPatientImageFromCamera": "uploading patient image from camera",
    "uploadingPatientImageFromGallery": "uploading patient image from gallery",
    "deletingOrderImageFromServer": "deleting order image from server",
    "uploadingOrderImageFromGallery": "uploading order image from gallery",
    "uploadingOrderImageFromCamera": "uploading order image from camera",
    "openingImageFromGrid": "opening image from grid",
    "openingMultipleImagesFromGrid": "opening multiple images from grid",
    "generatingPatientLink": "generating patient link",
    "gettingRemoteImagesList": "getting remote images list",
    "sendingUpdatesToServer": "sending updates to server",
    "deletingFile": "deleting file",
    "uploadingFile": "uploading file",

    // AI services
    "ai_services": "AI Services",
    "ai_services_desc":
        "Enable or disable AI-powered features such as receipt scanning, voice notes, and dental history extraction.",
    "on": "On",
    "off": "Off",
    "sameAsAppLanguage": "Same as app language",
    "audioTranscriptionLocale": "Audio transcription language",
    "audioTranscriptionLocale_desc":
        "By default, audio transcriptions are translated to the current app language. You can choose a different language if needed.",
    "no_training_privacy_info":
        "When you enable these features, your files are processed privately. We never store them after processing — they're read in memory only for your request (as streamed bytes) and discarded immediately after. Your data is never used for AI training.",

    // Receipt scanner
    "scanReceipt": "Scan Receipt",
    "readFromPhoto": "Read from photo",
    "readingItemsFromPhoto": "reading items from photo",
    "receiptScanFailed": "Receipt scan failed",
    "testing": "Testing connection...",
    "unknown": "Unknown",
    "unidentified": "Unidentified",

    // Dental history recording
    "voiceAutoFill": "Voice Auto-Fill",
    "dentalHistoryVoiceAutoFillHint":
        "Speak tooth numbers and conditions naturally to update the patient's chart automatically.",
    "postOperativeVoiceAutoFillHint":
        "Summarize the visit. Speak your charting, post-op notes, prescriptions, fees, or labwork to automatically fill out each form.",
    "transcribingYourAudio": "Transcribing your audio...",
    "recording": "Recording",
    "paused": "Paused",
    "processingDentalHistory": "processing dental history audio",
    "processingPostOpNotes": "processing post-op notes audio",

    // calendar system
    "calendarSystem": "Calendar system",
    "calendarSystem_desc":
        "Choose the calendar system for displaying dates in the calendar view",
    "gregorian": "Gregorian",
    "persian": "Persian (Jalali / Shamsi)",

    // Jalali month names
    "farvardin": "Farvardin",
    "ordibehesht": "Ordibehesht",
    "khordad": "Khordad",
    "tir": "Tir",
    "mordad": "Mordad",
    "shahrivar": "Shahrivar",
    "mehr": "Mehr",
    "aban": "Aban",
    "azar": "Azar",
    "dey": "Dey",
    "bahman": "Bahman",
    "esfand": "Esfand",

    // leads
    "importCsv": "Import CSV",
    "pickCsvFile": "Pick CSV file",
    "csvFile": "CSV file",
    "csvPasteHint": "Or paste CSV text below",
    "csvImportHowTo":
        "Use a header row with columns such as name, phone, email, source, campaign, interest, notes, stage. Extra columns are ignored. Existing phone numbers are skipped.",
    "leadsImported": "Leads imported",
    "leadsSkipped": "Rows skipped",
    "leadDetails": "Lead details",
    "leadStage": "Stage",
    "leadSource": "Source",
    "campaign": "Campaign",
    "interest": "Interest",
    "followUp": "Follow-up",
    "convertToPatient": "Convert to patient",
    "linkedPatient": "Linked patient",
    "openPatient": "Open patient",
    "scheduleAppointment": "Schedule appointment",
    "whatsappConsent": "WhatsApp consent",
    "leadStageNew": "New",
    "leadStageContacted": "Contacted",
    "leadStageInterested": "Interested",
    "leadStageScheduled": "Scheduled",
    "leadStageReschedule": "Reschedule",
    "leadStageConverted": "Converted",
    "leadStageLost": "Lost",
    "allStages": "All stages",
    "allSources": "All sources",
    "facebook": "Facebook",
    "instagram": "Instagram",
    "google": "Google",
    "walkIn": "Walk-in",
    "manual": "Manual",
    "clinicHours": "Clinic hours",
    "clinicHoursInfo":
        "Set morning and evening hours for each day. Turn off evening if that day is morning only. Booking skips the gap between sessions.",
    "slotLength": "Slot length (minutes)",
    "utcOffsetMinutes": "Timezone",
    "defaultBookingDoctor": "Default doctor for new bookings",
    "any": "Any",
    "opens": "Opens",
    "closes": "Closes",
    "morningHours": "Morning",
    "eveningHours": "Evening",
    "eveningSession": "Evening session",
    "fromTime": "From",
    "toTime": "To",
    "closed": "Closed",
    "closedAfternoon25": "Closed every afternoon 2:00–5:00 PM",
    "closedAfternoon25Short": "Closed 2–5 PM",
    "closedDates": "Closed dates (YYYY-MM-DD, comma separated)",
    "bookingApiToken": "n8n booking API token",
    "bookingApiTokenHint": "Send as header X-Booking-Token",
    "evolutionWhatsapp": "Evolution WhatsApp",
    "evolutionWhatsappInfo":
        "n8n uses these credentials with Evolution API. Message wording is in WhatsApp templates below.",
    "evolutionBaseUrl": "Evolution base URL",
    "evolutionApiKey": "Evolution API key",
    "evolutionInstance": "Instance name",
    "clinicName": "Clinic name",
    "whatsappConfirmTemplate": "Confirmation message",
    "whatsappRemindTemplate": "Reminder message",
    "freeSlots": "Next free slots",
    "noFreeSlots": "No free slots in the next two weeks. Check clinic hours in Settings.",
    "markInterestedToBook": "Set stage to Interested to book a visit",
    "bookVisitSlots": "Book a 20-minute visit",
    "threeSlotsPerHour": "Each visit is 20 minutes (3 per hour)",
    "bookedToday": "Booked today",
    "nextFreeSlots": "Free slots",
    "pickDoctor": "Doctor",
    "slotTaken": "That slot was just taken. Pick another.",
    "sendWhatsappConfirm": "Send WhatsApp confirmation",
    "outreachDashboard": "Outreach",
    "birthdays": "Birthdays",
    "birthdayToday": "Birthday today",
    "daysAway": "Days away",
    "noBirthdays": "No upcoming birthdays with a full date and phone number",
    "whatsappCampaign": "WhatsApp campaign",
    "whatsappCampaignInfo":
        "Flyers, Google review link, treatments the AI may name, and quiet hours. Canned WhatsApp copy is under WhatsApp templates.",
    "googleReviewUrl": "Google review link",
    "openrouterModel": "OpenRouter model",
    "sendFlyerWithChat": "Allow the assistant to attach the flyer in a chat",
    "uploadFlyer": "Upload flyer image",
    "flyerHint":
        "PNG or JPG. n8n sends this image only to the WhatsApp number in that conversation.",
    "birthdayTemplate": "Birthday message",
    "reviewTemplate": "Google review request",
    "reviewFollowUp": "Review follow-up",
    "reviewAsked": "Times asked",
    "markReviewDone": "Mark review received",
    "rescheduleRequested": "Wants to reschedule",
    "interestedQueue": "Interested (call to confirm)",
    "googleMapsUrl": "Google Maps link",
    "staffWhatsappGroup": "Staff WhatsApp group ID",
    "marketingInstance": "Marketing WhatsApp instance",
    "marketingInstancesHint":
        "Leave extra instances empty until those WhatsApp numbers are connected in Evolution. Marketing always uses the clinic instance first, then 1–4 as you fill them in (up to 5 lines, rotating). Type Evolution instance names, not phone numbers. Add those accounts to the staff leads group. Do not chat in that group. Appointments, birthdays, reviews, and group alerts always use the clinic instance only.",
    "marketingImageUrl": "Marketing image URL",
    "queueMarketing": "Send image to selected",
    "leadCalled": "Called",
    "coming": "Coming",
    "noVisitYet": "No visit yet",
    "clinicAddress": "Clinic address",
    "campaignMessages": "Campaign messages",
    "pickCampaignMessage": "Choose a saved message",
    "newCampaignMessage": "New message",
    "campaignMessageBody": "Message text. Use {name} {clinic} {phone} {address} {maps} {date} {day} {time}",
    "templatePlaceholders":
        "Placeholders: {name} {clinic} {phone} {address} {maps} {date} {day} {time}. Never write prices here.",
    "mergeLeads": "Merge selected",
    "callOutcome": "Call outcome",
    "callNoAnswer": "No answer",
    "callWrongNumber": "Wrong number",
    "callCallback": "Call back later",
    "callBooked": "Booked",
    "callNotInterested": "Not interested",
    "assistantOff": "Assistant off (staff talking)",
    "whatsappSendError": "WhatsApp send failed",
    "treatmentsNoPrices": "Treatments the assistant may name (no prices)",
    "quietHours": "Quiet hours (start – end, clinic time)",
    "whatsappTemplates": "WhatsApp templates",
    "whatsappTemplatesInfo":
        "Canned WhatsApp for welcome, confirm, welcome+confirm, reminder, aftercare, prescription, no-show, reschedule, birthday, review, staff alerts, and opt-out. The AI only chats. Placeholders: {name} {clinic} {phone} {address} {maps} {date} {day} {time} {note} {reviewUrl}. Clinic name is always bold. Google may still show its own link preview title for review URLs — that is controlled by Google, not the message text.",
    "tplWelcome": "Welcome (walk-in)",
    "tplWelcomeConfirm": "Welcome + confirmation (one message)",
    "tplHistory": "Medical / dental history",
    "tplAftercare": "After your visit (same day done)",
    "tplNoShow": "No-show",
    "tplReschedule": "Reschedule / time changed",
    "tplPrescription": "Prescription (after visit)",
    "tplLeave": "Leave clinic (welcome + Rx + thanks)",
    "tplNextBrief": "Next appointment (brief, delayed)",
    "tplAlertNew": "Staff alert — new lead",
    "tplAlertBack": "Staff alert — returning",
    "tplOptOut": "Unsubscribed",
    "resetDefaults": "Reset to defaults",
    "prescriptions": "Prescriptions",
    "prescriptionsTabInfo": "Manage the medicine list",
    "addMedicine": "Add medicine",
    "medicineName": "Medicine name",
    "searchMedicines": "Search medicines",
    "noMedicinesFound": "No medicines found",
    "resetMedicinesConfirm":
        "Replace the clinic medicine list with the built-in dental defaults?",
    "addPrescription": "Add prescription",
    "doseFrequency": "Dose / frequency",
    "doseTimes": "Take medicine",
    "doseMorning": "Morning",
    "doseAfternoon": "Afternoon",
    "doseEvening": "Evening",
    "mealTiming": "With food",
    "beforeMeals": "Before meal",
    "afterMeals": "After meal",
    "withMeals": "With meals",
    "anytime": "Anytime",
    "sendPrescriptionWhatsApp": "Send on WhatsApp",
    "sendOnWhatsApp": "WhatsApp",
    "openPrescriptionsCatalog": "Prescriptions catalog",
    "patientPrescriptionsInfo":
        "Prescriptions from this patient’s visits. Edit opens the appointment Prescription tab. The medicine catalog is under the Prescriptions nav item.",
    "noPrescriptionsYet": "No prescriptions recorded for this patient yet",
    "patientIntakeSource": "How did they come in?",
    "intakeWalkIn": "Walk-in at clinic",
    "intakePhoneCall": "We called / booked on phone",
    "whatsappHoldPatient": "Hold WhatsApp messages",
    "whatsappHoldPatientHint":
        "No reminders, leave messages, or next-visit texts until you turn this off.",
    "treatmentCompleted": "Treatment completed",
    "treatmentCompletedHint":
        "Check this when the full course of care is finished. Then use Send review request below — reviews are not sent automatically.",
    "sendReviewRequest": "Send review request",
    "reviewRequestSent": "Review request sent on WhatsApp",
    "reviewRequestSkipped":
        "Could not send. Check phone, WhatsApp hold, treatment completed, and that it was not already sent.",
    "reviewAlreadySent": "Review request already sent for this patient.",
    "reviewAlreadyReceived": "Patient already marked as having left a review.",
    "sendReviewFromPatient": "Send from patient details when ready",
    "isNoShow": "No-show (missed visit)",
    "clinicPhonePatients": "Primary clinic phone (patients call this)",
    "clinicPhonePatientsHint":
        "Shown in WhatsApp as {phone} for appointments and enquiries. Same value as Phone at the top of Settings.",
    "invoices": "Invoices",
    "invoice": "Invoice",
    "generateInvoice": "Generate invoice",
    "generateInvoices": "Generate missing invoices",
    "noInvoicesYet": "No billed visits for this patient yet",
    "patientInvoicesInfo":
        "Invoices are generated from visit price and payment. Extra paid is kept as credit and deducted from the next treatment.",
    "creditBalance": "Account credit",
    "creditApplied": "Credit applied",
    "creditAppliedNextHint":
        "This extra amount is deducted from the next treatment. Paid today is only what is still due.",
    "amountDue": "Amount due",
    "invoiceBalance": "Balance",
    "printInvoice": "Print invoice",
    "regenerateInvoice": "Update from visit",
    "description": "Description",
    "noBirthdaysForThisDay": "No birthdays on this day",
  };
}
