...
import 'package:chatdent/services/dicom/dicom_file_cache.dart';
import 'package:chatdent/services/dicom/dicom_importer.dart';
import 'package:chatdent/services/dicom/dicom_io_service.dart';
...
class _FakeLinksStore {
  ...
  Future<void> setPatient(String dicomPatientId, String chatdentPatientId) async {
    _patientLinks[dicomPatientId] = chatdentPatientId;
  }
...
class _Harness {
  ...
  /// In-memory appointment dates per ChatDENT patient (id dates), used by the
  ...
  DicomImporter rebuild() {
    return DicomImporter(
      ...
      setPatient: (dicomId, chatdentId) async {
        persistenceEvents.add('patient:$dicomId:$chatdentId');
        if (failPatientLink) {
          throw StateError('fake patient-link persistence failed');
        }
        fakeLinks._patientLinks[dicomId] = chatdentId;
      },
...
void main() {
  ...
  group('scanAndBuildPending matching', () {
    test('auto-link via dicomPatientLinksMap (confidence 1.0, isConfirmed)',
      ...
      final h = _newHarness();
      final patient =
          Patient.fromJson({'id': 'chatdent-1', 'title': 'John Smith'});
      h.patients['chatdent-1'] = patient;
      h.fakeLinks._patientLinks['P100'] = 'chatdent-1';
      ...
      expect(p.matchedPatient?.id, 'chatdent-1');
    });

    test('fuzzy name match surfaces a suggestion with computed confidence',
        () async {
      final h = _newHarness();
      // ChatDENT patient whose name tokenizes identically to the DICOM name.
      final patient =
          Patient.fromJson({'id': 'chatdent-1', 'title': 'John Smith'});
      h.patients['chatdent-1'] = patient;
      ...
      h.appointmentDates['chatdent-1'] = {DateTime(2025, 1, 1)};
    ...
    test('no plausible match matchedPatient null, confidence 0', () async {
      final h = _newHarness();
      h.patients['chatdent-1'] = Patient.fromJson(
          {'id': 'chatdent-1', 'title': 'Completely Different Name'});
    ...
    test('dates boost confidence when appointments match study dates',
        () async {
      final h = _newHarness();
      h.patients['chatdent-1'] =
          Patient.fromJson({'id': 'chatdent-1', 'title': 'John Smith'});
      h.patients['chatdent-2'] =
          Patient.fromJson({'id': 'chatdent-2', 'title': 'John Smith Jr'});
      // Both patients have the name "John Smith", but only chatdent-1 has an
      ...
      // chatdent-1 should win because dates match.
      expect(pending.single.matchedPatient?.id, 'chatdent-1');
    ...
    test('dates break tie when two patients have similar names', () async {
      ...
      h.patients['chatdent-1'] =
          Patient.fromJson({'id': 'chatdent-1', 'title': 'John Smith'});
      h.patients['chatdent-2'] =
          Patient.fromJson({'id': 'chatdent-2', 'title': 'James Smith'});
      // Only James Smith has an appointment on the study date.
      h.appointmentDates['chatdent-2'] = {DateTime(2025, 1, 1)};
    ...
    test('stale link (patient gone) falls through to fuzzy matching', () async {
      ...
      h.fakeLinks._patientLinks['P100'] = 'gone-chatdent-id';
  ...
  group('full DICOM approval workflows', () {
    test('linked patient import creates a new appointment automatically',
      ...
      final h = _newHarness();
      h.patients['chatdent-1'] =
          Patient.fromJson({'id': 'chatdent-1', 'title': 'Linked Patient'});
      h.fakeLinks._patientLinks['P100'] = 'chatdent-1';
      h.addFile(_FakeFile(
      ...
      expect(created.patientID, 'chatdent-1');
    ...
    test('linked patient import reuses an existing same-day appointment',
        () async {
      final h = _newHarness();
      h.patients['chatdent-1'] = Patient.fromJson({'id': 'chatdent-1'});
      h.fakeLinks._patientLinks['P100'] = 'chatdent-1';
      final existing =
          Appointment.fromJson({'id': 'existing', 'patientID': 'chatdent-1'})
    ...
    test('unmatched scan remains pending until manually matched and approved',
      ...
      final patient = Patient.fromJson({'id': 'chatdent-manual'});
      ...
      expect(h.appointments.values.single.patientID, 'chatdent-manual');
  ...
  group('approveImport', () {
    test('creates one appointment per study date with correct dcmImgs',
      ...
      final pending = await h.importer.scanAndBuildPending();
      await h.importer.approveImport(pending.single, chatdentPatientId: 'chatdent-1');

      // Two appointments created one per date.
      final created =
          h.appointments.values.where((a) => a.patientID == 'chatdent-1').toList();
      ...
      expect(h.fakeLinks._patientLinks['P100'], 'chatdent-1');
    });
    ...
    test('new appointment is persisted before its upload starts', () async {
      ...
      await h.importer.approveImport(pending, chatdentPatientId: 'chatdent-1');
    ...
    test('appends to an existing same-day appointment (no new appointment)',
      ...
      final existing =
          Appointment.fromJson({'id': 'ap-exist', 'patientID': 'chatdent-1'})
    ...
    test('throws when no target patient resolved', () async {
      ...
      final pending = await h.importer.scanAndBuildPending();
      // No matchedPatient, no chatdentPatientId must throw.
      expect(
    ...
    test('failed file processing rolls back its imported marker', () async {
      ...
      expect(rediscovered.single.matchedPatient?.id, 'chatdent-1');
    });
    ...
    test('patient-link persistence is awaited before file processing',
      ...
      final patientIndex = h.persistenceEvents
          .indexWhere((event) => event == 'patient:P100:chatdent-1');
    ...
    test('patient-link rollback failure throws DicomApprovalRollbackException',
      ...
      await expectLater(
        h.importer.approveImport(pending, chatdentPatientId: 'chatdent-1'),
    ...
    test('link persistence failure rolls back every imported marker', () async {
      ...
      await expectLater(
        () =>
            h.importer.approveImport(pending.single, chatdentPatientId: 'chatdent-1'),
        throwsA(isA<StateError>()),
      );
      expect(h.fakeLinks._registry, isEmpty);
  ...
  group('DicomApprovalResult and pending-item reduction', () {
    ...
    test('copyWithFiles retains match metadata and recalculates names/dates',
        () {
      final patient = Patient.fromJson({'id': 'chatdent-1', 'title': 'ChatDENT'});
      ...
      final original = DicomPendingImport(
        ...
        matchedPatientId: 'chatdent-1',
        matchedPatientName: 'ChatDENT',
        confidence: .8,
        isConfirmed: true,
        autoLinked: true,
      );
      ...
      expect(reduced.matchedPatientId, 'chatdent-1');
      expect(reduced.matchedPatientName, 'ChatDENT');
  ...
  group('unmarkPending (recovery / undo)', () {
    ...
    test('pending manual match survives rescan but does not auto-import',
      ...
      h.fakeLinks._pendingMatches['P100'] = 'chatdent-1';
    ...
    test('pending match ignored if ChatDENT patient was deleted', () async {
      final h = _newHarness();
      // Pending match references a non-existent patient.
      h.fakeLinks._pendingMatches['P100'] = 'deleted-chatdent-id';
