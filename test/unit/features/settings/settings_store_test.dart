...
void main() {
  ...
  group('GlobalSettings', () {
    ...
    test('defaults map has expected keys', () {
      ...
      expect(globalSettings.defaults, contains('clinic_hours___'));
      expect(globalSettings.defaults, contains('evo_settings___'));
      expect(globalSettings.defaults, contains('n8n_book_token_'));
      expect(globalSettings.defaults, contains('wa_campaign____'));
      expect(globalSettings.defaults, contains('wa_flyer_______'));
    });

    test('every global setting id is exactly 15 PocketBase-safe characters', () {
      for (final id in globalSettings.defaults.keys) {
        expect(id.length, 15, reason: '"$id" is ${id.length} chars');
