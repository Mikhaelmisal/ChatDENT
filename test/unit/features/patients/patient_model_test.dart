...
void main() {
  group('Patient.fromJson', () {
    ...
    test('india numbers persist as 91 plus 10 digits with no spaces', () {
      final p = Patient.fromJson({
        'phone': '+91 98765 43210',
      });
      expect(p.phonesString, '919876543210');
      expect(p.toJson()['phone'], '919876543210');
    });
  ...
  group('Patient computed getters', () {
    ...
    test('age uses full birthdate when stored as YYYYMMDD', () {
      final now = DateTime.now();
      int encode(DateTime d) => d.year * 10000 + d.month * 100 + d.day;
      final had = testPatient(id: 'age-full-1', birth: encode(DateTime(2000, 1, 1)));
      final notYet = testPatient(id: 'age-full-2', birth: encode(DateTime(2000, 12, 31)));
      expect(had.age, now.year - 2000);
      final expectedNotYet =
          (now.month == 12 && now.day == 31) ? now.year - 2000 : now.year - 2001;
      expect(notYet.age, expectedNotYet);
      expect(had.hasFullBirthDate, isTrue);
      expect(had.birthDateString, '01/01/2000');
    });

    test('tryParseBirthDate accepts DD/MM/YYYY', () {
      expect(Patient.tryParseBirthDate('15/03/1990'), DateTime(1990, 3, 15));
      expect(Patient.tryParseBirthDate('31/02/1990'), isNull);
      expect(Patient.tryParseBirthDate('1990'), isNull);
    });
