...
import 'package:chatdent/services/dicom/dicom_orphans_cleaner.dart';
...
void main() {
  ...
  group('PocketBase record id and JSON field helpers', () {
    test('accepts exactly 15 alphanumeric/underscore characters', () {
      expect(isPocketBaseRecordId('clinic_hours___'), isTrue);
      expect(isPocketBaseRecordId('n8n_book_token_'), isTrue);
      expect(isPocketBaseRecordId('evo_settings___'), isTrue);
      expect(isPocketBaseRecordId('wa_campaign____'), isTrue);
      expect(isPocketBaseRecordId('wa_flyer_______'), isTrue);
    });

    test('rejects ids that would fail PocketBase max=15', () {
      expect(isPocketBaseRecordId('n8n_booking_token'), isFalse);
      expect(isPocketBaseRecordId('clinic_hours____'), isFalse);
      expect(isPocketBaseRecordId('short'), isFalse);
    });

    test('decodes a stored JSON object string', () {
      final decoded = decodeStoredJsonField('{"id":"clinic_hours___","value":"{}"}');
      expect(decoded, isA<Map>());
      expect(decoded['id'], 'clinic_hours___');
    });

    test('decodes a double-encoded JSON string', () {
      final decoded = decodeStoredJsonField('"{\\"a\\":1}"');
      expect(decoded, {'a': 1});
    });
  });
}
