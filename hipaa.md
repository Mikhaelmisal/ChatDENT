# ChatDENT HIPAA & GDPR Compliance Plan

---

## Preface: About ChatDENT

### What is ChatDENT?

ChatDENT is an **open-source dental clinic management system** (GPLv3). It handles patient records, appointment scheduling, dental charts, X-ray/DICOM imaging, lab work tracking, expense management, clinic notes, and multi-user collaboration — all from a single application.
...
| **AI Services** | External Cloudflare Worker (`ai.chatdent.app`) | Dental history transcription, receipt scanning, post-op notes generation |
...
**AI Processing Data**:
- Audio recordings of dental examinations → `ai.chatdent.app`
- Receipt/invoice images → `ai.chatdent.app`
- Appointment data → `ai.chatdent.app`
...
| Client ↔ AI Worker (`ai.chatdent.app`) | HTTPS. Audio files, receipt images, and appointment data sent to external Worker. | **HIGH** — PHI sent to third-party service with no BAA. |
| Client ↔ Push Relay (`notifications.chatdent.app`) | HTTPS. Push payloads contain patient-readable identifiers and appointment data. | **HIGH** — PHI flows through Cloudflare Worker. |
...
| Patient Web Portal (`web.chatdent.app`) | HTTPS. Patient data including appointments, payments, and photos accessible via base64-encoded URL parameter (no authentication). | **HIGH** — Anyone with the link can view full patient data. |
...
| **Patient portal** | URL format: `https://web.chatdent.app/{base64(patientID|name|server|relayKey)}`. No authentication. | Anyone with the URL has full access. No expiration. No access revocation. |
...
1. The Cloudflare Worker relay (`notifications.chatdent.app`)
...
Three AI services send PHI to `https://ai.chatdent.app`:
...
1. Implement certificate pinning for:
   ...
   - `ai.chatdent.app` (AI Worker)
   - `notifications.chatdent.app` (Push Relay)
...
| Component | Change |
|-----------|--------|
| AI Worker (`ai.chatdent.app`) | Add DPA compliance, data retention limits, delete endpoint, audit logging |
...
| Cloudflare Pages (web.chatdent.app) | Add security headers, CSP |
...
*This compliance plan was prepared on 2026-07-31 based on a thorough review of the ChatDENT codebase at version 0.13.0. It should be reviewed and updated as the codebase evolves and as regulatory requirements change.*
