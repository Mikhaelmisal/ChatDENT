---
description: Cross-platform patterns for ChatDENT — conditional imports for web vs native, platform checks, JS bridge, image handling across platforms, file system access, and audio recording platform abstractions.
applyTo: "lib/utils/**"
---

# ChatDENT Cross-Platform Patterns

## Platform Targets

ChatDENT runs on **Android, iOS, Web, macOS, and Windows** from a single codebase. Platform differences are handled through three mechanisms:
...
ChatDENT uses Dart's **conditional imports** to swap implementations at compile time. The pattern always uses three files:
...
| **Upload source** | File path on disk | Blob URL or proxied through `https://imgs.chatdent.app/` |
...
- **Web special case**: Blob URLs are fetched via `http.get()` and sent as bytes; external URLs are proxied through `https://imgs.chatdent.app/`
...
```dart
import 'package:chatdent/utils/safe_dir.dart';
