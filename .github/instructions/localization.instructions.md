---
description: Localization system for ChatDENT — how to add translations, create new locales, audit keys, and handle RTL. Applies when working with UI strings, user-facing text, locale files, or the txt()/Txt localization API.
applyTo: "lib/services/localization/**"
---

# ChatDENT Localization System

## Architecture

ChatDENT uses a **custom dictionary-based localization system** (not Flutter's `flutter_localizations`). The system is lightweight and designed for a dental clinic management app with English, Arabic, Spanish, and Greek support.
